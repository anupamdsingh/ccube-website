import React from "react";
import Buttons from "@/src/components/buttons";
import Link from "next/link";
import Navbar from "@/src/components/navbar";

function home() {
  return (
    <div className="h-screen w-screen">
      <div>
        <Navbar />
      </div>
      {/* <img className=" h-full w-full" src="https://cdn.wallpapersafari.com/7/83/Do83Mu.jpg" alt=""/> */}
      <div className=" h-3/6 w-screen bg-white">
        <div className="text-4xl pt-32 px-20 ">
          Welcome to the{" "}
          <b>
            <i>
              {" "}
              C <sup>cube</sup>
            </i>
          </b>
        </div>
        <div className="text-5xl px-20 text-indigo-500">
          <i> Where Innovation Meets Collaboration! </i>
        </div>
        <div className="text-2xl px-20 text-slate-700">
          As a coding club, our mission is simple: to foster a collaborative
          environment where members can learn, share, and grow their coding
          skills.
        </div>
      </div>

      {/* IMG AND WHO WE ARE */}
      <div className="flex w-screen bg-white">
        <div className="w-1/2 flex justify-end items-center">
          <div className="px-12">
            <img
              src="data:image/png;base64,"
              alt="img"
            />
          </div>
        </div>
        <div className="w-1/2 h-full justify-end items-end">
          <div className="w-8/12 text-4xl pt-9 flex justify-center items-center text-slate-700">
            <b>Who we are?</b>
          </div>
          <div className="w-8/12 text-2xl px-4 pt-9 justify-center items-center text-slate-600">
            We are a group of students and professionals and hobbyists who are
            passionate about coding. Our club welcomes members from all walks of
            life, regardless of their coding experience or background. Whether
            you are a beginner taking your first steps in coding or an
            experienced programmer looking to expand your skills, we have a
            place for you in our club, you can join us and learn. <br />
          </div>
        </div>
      </div>
      {/* what we do */}
      <div className="flex w-screen bg-white ">
        <div className=" w-1/2 h-full bg-white justify-start items-center">
          <div className=" text-slate-700">
            <div className="w-8/12 text-4xl pt-36 flex justify-end items-center">
              <b>What we do?</b>
            </div>
          </div>
          <div className="flex text-2xl pt-9 justify-end items-center text-slate-600">
            <div className="w-8/12 flex justify-start">
              Our club hosts events like meetups, workshops, hackathons, and
              coding challenges for learning, collaboration, and skill
              development. We cover topics such as web and mobile app
              development, machine learning, and AI. Guest speakers from the
              industry share valuable insights. We create a supportive and
              inclusive environment for members to express themselves and
              receive feedback to enhance their coding skills.
            </div>
          </div>
        </div>
        <div className="w-1/2 pt-11 bg-white flex justify-start items-center">
          <div className="w-8/12 flex item-center ">
            <img
              src="https://st2.depositphotos.com/1051392/5967/i/950/depositphotos_59675283-stock-photo-3d-man-what-we-do.jpg"
              alt=""
            />
          </div>
        </div>
      </div>
      {/* why join us*/}
      <div className=" w-screen flex justify-center items-center bg-white pt-11">
        <div className="w-3/4 h-full bg-white justify-center items-center">
          <div className="text-4xl pt-9 flex justify-center items-center text-slate-700">
            <b>Why join us?</b>
          </div>
          <div className="text-2xl pb-10 px-4 pt-9 justify-center items-center text-slate-600">
            Learning Opportunities: Members acquire coding skills, expand
            knowledge, and stay up-to-date with industry trends in a supportive
            environment.
            <br />
            <br />
            Collaboration and Networking: The club fosters a collaborative
            community where members connect, collaborate on coding projects, and
            expand their network.
            <br />
            <br />
            Access to Resources: Members have coding tools, libraries,
            tutorials, and documentation for support.
            <br />
            <br />
            Fun and Camaraderie: Our club organizes social events, game nights,
            and coding challenges to foster friendships and create an enjoyable
            atmosphere.
            <br />
            <br />
            Join our coding club today and embark on an exciting journey
            exploring the world of coding together
            <br />
          </div>
        </div>
      </div>
      {/* <div className=" h-1/2 flex justify-center bg-indigo-500">
        <div>
          <div
            className="order-2 md:order-3 flex flex-wrap items-center justify-end mr-0 md:mr-4"
            id="div-content"
          >
            <div className="auth flex items-center w-full md:w-full text-indigo-500">
              <div className="px-2 pt-2">
                <Buttons type="primary" Name="Sign-in" />
              </div>
              <div className="px-2 pt-2">
                <Buttons type="primary" Name="Sign-up" />
              </div>
            </div>
          </div>
        </div>
      </div> */}
    </div>
  );
}
export default home;
